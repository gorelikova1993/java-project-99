...
public class TaskStatusControllerTest {
    ...
    void testGetTaskStatusById() throws Exception {
        var result = mockMvc.perform(get("/api/task_statuses/{id}", taskStatus.getId()).with(jwt()))
                .andExpect(status().isOk())
                .andReturn();
    ...
    void testDeleteTaskStatus() throws Exception {
        mockMvc.perform(delete("/api/task_statuses/{id}", taskStatus.getId()).with(jwt()))
    ...
    void testGetTaskNotFound() throws Exception {
        mockMvc.perform(get("/api/task_statuses/{id}", 9999).with(jwt()))
